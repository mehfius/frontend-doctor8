  ...
  $dir       = getcwd()."/".$ext."/src/";
	...
	while ($arquivo = readdir($ponteiro)){
      ...
      if(!in_array($arquivo,['.','..']) ){
        ...
        $subdir       = getcwd()."/".$ext."/src/".$arquivo;
	      $subponteiro  = opendir($subdir);
        
        while ($arquivo2 = readdir($subponteiro)){
          
          if(!in_array($arquivo2,['.','..']) && substr($arquivo2,-(strlen($ext)+1))==(".".$ext)){
          //echo "$arquivo2";
            $array[]=$subdir."/".$arquivo2;
